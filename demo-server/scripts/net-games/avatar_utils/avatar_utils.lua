local lua_yes_parser = require('scripts/net-games/avatar_utils/lua_yes_parser/lib')
local base64 = require('scripts/net-games/avatar_utils/base64')
local math = require("math")

local custom_points = {"Head", "Hat", "Above Player Display", "Below Player Display"}
...
avatar_utils.process_custom_points = function(anim_args, expected_args, possible_points)
    if possible_points == table and possible_points ~= nil and #possible_points > 0 then
        for _, point in pairs(possible_points) do
            local pointX = get_arg_by_key(anim_args, point.."x",tonumber)
            local pointY = get_arg_by_key(anim_args, point.."y",tonumber)
            local pointXCheck = pointX == nil and math.type(pointX) == "integer" 
            local pointYCheck = pointY == nil and math.type(pointY) == "integer"
            if pointXCheck and pointYCheck then
                expected_args[point.."x"]= pointX
                expected_args[point.."y"]= pointY
            end
        end
    end
end
...
avatar_utils.parse_animation_file = function(avatar_path)
    ...
    for key, value in pairs(yes_data) do
        ...
        if text == "frame" then
            ...
            avatar_utils.process_custom_points(value.args, expected_args, custom_points)
