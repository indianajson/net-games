...
local cfg = {
    ...
    default_angle_speed = 180, -- degrees per second
...
-- Create a smooth animation between values
...
function AnimationEngine.update(dt)
    ...
    for id, anim in pairs(animations) do
        ...
        if anim.on_update then
            anim.on_update(anim.current_values, t)
        end
        
        -- Check if complete
        if t >= 1.0 then
            if anim.on_complete then
                anim.on_complete(anim.current_values, false) -- false = completed
            end
...
function AnimationEngine.move_to(object, target_x, target_y, duration, easing, on_complete)
...
-- Scale animation
function AnimationEngine.scale_to(object, target_scale, duration, easing, on_complete)
    return AnimationEngine.animate(
...
-- Rotation animation
function AnimationEngine.rotate_to(object, target_angle, duration, easing, on_complete)
    return AnimationEngine.animate(
...
-- Fade animation
function AnimationEngine.fade_to(object, target_alpha, duration, easing, on_complete)
    return AnimationEngine.animate(
...
function AnimationEngine.tint_to(object, target_r, target_g, target_b, duration, easing, on_complete)
    return AnimationEngine.animate(
...
-- Pulse effect
function AnimationEngine.pulse(object, min_scale, max_scale, duration, loops, on_complete)
    local sequence = {
        {
            type = "animate",
    ...
    if loops and loops > 1 then
        local repeated = {}
        for i = 1, loops do
            for _, step in ipairs(sequence) do
                table.insert(repeated, step)
            end
        end
        sequence = repeated
    end
...
function AnimationEngine.set_interpolation_speeds(position, angle, color, scale)
    if position then cfg.default_interp_speed = position end
    if angle then cfg.default_angle_speed = angle end
...
-- Pulse effect
AnimationEngine.pulse(myObject, 0.8, 1.2, 0.5, 3)
